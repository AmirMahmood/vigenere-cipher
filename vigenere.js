...
function kasiskiExamination(ctext, sequence_len = 3) {
    ...
    // sort
    var sorted_factor = [];
    for (var z in factor_dict) {
        sorted_factor.push([z, factor_dict[z]]);
    }
    sorted_factor.sort(function (a, b) {
        return b[1] - a[1];
    });
...
function keyDetection(ctext, key_len) {
    ...
    const f = (a, b) => [].concat(...a.map(d => b.map(e => [].concat(d, e).join(""))));
    const cartesian = (a, b, ...c) => (b ? cartesian(f(a, b), ...c) : a);

    return cartesian(...key_chars_candidate);
}